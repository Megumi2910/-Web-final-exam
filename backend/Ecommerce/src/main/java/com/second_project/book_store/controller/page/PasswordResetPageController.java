...
public class PasswordResetPageController {
     ...
     * 
     * EXCEPTION HANDLING (Best Practice - Balanced Approach):
     * - Validation errors (password length, matching passwords) → Handled by @Valid + BindingResult
     * - ExpiredTokenException → Caught HERE to show error on form (better UX - user stays on page)
     * - InvalidPasswordException → Bubbles to PageExceptionHandler (already handles reset-password path specially)
     * - ResetPasswordTokenNotFoundException → Bubbles to PageExceptionHandler (shows error page)
     * - Other exceptions → Bubbles to PageExceptionHandler (shows generic error page)
     * 
     * WHY THIS APPROACH?
     * - ExpiredTokenException is caught here because showing it on the form provides better UX
     *   (user sees error in context, can request new link from same page)
     * - Other exceptions bubble up because PageExceptionHandler already handles them appropriately
     * - This balances DRY principle with user experience
     * 
    ...
    public String processResetPassword(
        ...
        try {
            ...
            // Throws: ExpiredTokenException, InvalidPasswordException, ResetPasswordTokenNotFoundException
            ...
            // CAUGHT HERE: Show error on form for better UX
            // User sees error in context and can request a new reset link
            model.addAttribute("token", token);
            model.addAttribute("error", ex.getMessage());
            return "reset-password";
        }
        // Other exceptions (InvalidPasswordException, ResetPasswordTokenNotFoundException) 
        // bubble up to PageExceptionHandler which handles them appropriately
    }
}
